...
class LessonsTestCase(test.APITestCase):
    ...
    def test_get(self):
        response = self.client.get(f'/lessons/{self.lesson_id}/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_put(self):
        response = self.client.put(f'/lessons/{self.lesson_id}/update/', {
            'title': 'Default lesson',
            'description': 'New default lesson description',
            'course': self.course_id
        })
    ...
    def test_patch(self):
        response = self.client.patch(f'/lessons/{self.lesson_id}/update/', {
            'description': 'New default lesson description'
        })
        self.assertEqual(response.status_code, status.HTTP_202_ACCEPTED)
        self.assertEqual(response.json()['description'], 'New default lesson description')
    ...
    def test_delete(self):
        response = self.client.delete(f'/lessons/{self.lesson_id}/delete/')
